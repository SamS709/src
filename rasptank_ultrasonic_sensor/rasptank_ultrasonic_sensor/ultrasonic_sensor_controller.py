...
class UltrasonicSensorController(Node):
    ...
    def __init__(self):
        ...
        self.backup_speed = -0.1       # Backup speed when too close (m/s)
        self.turn_speed = 0.5          # Angular speed for turning (rad/s)
    ...
    def control_callback(self):
        ...
        if self.state == "forward":
            if self.current_distance <= self.stop_distance:
                ...
                linear_speed = self.calculate_speed(self.current_distance)
            ...
            cmd_vel.angular.z = self.turn_speed  # Turn right (positive angular velocity)
            self.turn_counter -= 1
        ...
        if self._log_counter % 40 == 0:  # Log every 2 seconds
            self.get_logger().info(f'State: {self.state}, Distance: {self.current_distance:.1f} cm, Speed: {cmd_vel.linear.x:.2f} m/s')
